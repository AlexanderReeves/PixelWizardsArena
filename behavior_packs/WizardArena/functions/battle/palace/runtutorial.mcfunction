...
#summon the dummy
execute as @e[type=space:variables,tag=noorbs,scores={tutorialstate=1}] run tellraw @a {"rawtext":[{"text":"§fWizard Wally: §dNicely done! Now equip your staff and walk into the dummy... You'll will attack automatically as you move."}]}
...
#CREATE THE PRSCTISE TUTORIAL DUMMY
...
execute as @e[type=space:variables,tag=noorbs,scores={tutorialstate=1}] run tag @e[type=space:healthbar,tag=!tagged] add enemy
...
execute as @e[type=space:variables,scores={tutorialstate=2..4}] run execute at @e[type=space:dummy,scores={dummydirectiontimer=0..199}] run tp @e[type=space:dummy] ~ ~ ~-0.05 180 ~
#When the score is 70 to 140, go backwards
execute as @e[type=space:variables,scores={tutorialstate=2..4}] run execute at @e[type=space:dummy,scores={dummydirectiontimer=200..400}] run tp @e[type=space:dummy] ~ ~ ~0.05 0 ~

#increase tick int score
execute as @e[type=space:variables,scores={tutorialstate=2..4}] run scoreboard players add @e[type=space:dummy] dummydirectiontimer 1
#reset the score back to 0 when completed
execute as @e[type=space:variables,scores={tutorialstate=2..4}] run scoreboard players set @e[type=space:dummy,scores={dummydirectiontimer=400}] dummydirectiontimer 0
...
execute as @e[type=space:variables,scores={tutorialstate=3}] run execute as @a[scores={magiccharge=0}] run execute unless entity @e[type=wizard:lightning] run summon wizard:lightning -145.33 207.20 363.56
...
#Load tutorial part 5/end when the dummy dies again
execute as @e[type=space:variables,scores={tutorialstate=4}] run tag @s add dummyalive
execute as @e[type=space:variables,scores={tutorialstate=4}] run execute as @e[type=space:healthbar,scores={hb=..0}] run tag @e[type=space:variables] remove dummyalive
execute as @e[type=space:variables,tag=noorbs,scores={tutorialstate=4},tag=!dummyalive] run function battle/palace/tutorialp5


#end tutorial early if the player walks to the quit portal
execute as @e[type=wizard:wizard] at @s run execute as @e[type=space:portal,r=1] run function battle/palace/endtutorial
